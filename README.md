...
[![Version 6.23.0-rc.1](https://img.shields.io/badge/version-6.23.0--rc.1-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.23.0-rc.1 | 2026-06-08 | **feat(subagents): adaptive progress-aware planning-swarm wait (completes the swarm fan-out/waits set).** `plan_task` planning scouts no longer fail to a fixed cutoff that discards in-flight work: `_collect_planning_handoffs` polls in slices and keeps extending while at least one non-terminal scout is RUNNING with a fresh heartbeat (from `queue_snapshot.json`), up to a generous `OUROBOROS_PLAN_TASK_SWARM_MAX_WAIT_SEC` ceiling (default 900s), then fails closed with a precise `wait_stop_reason` (`stalled`/`saturated`/`ceiling`). The wait/stop decision is a structured fact (terminal status + heartbeat freshness), not keyword matching (P5). Adds a "Timeout & Wait Control" guideline to DEVELOPMENT.md and a pointed progress-aware-waiting line to CHECKLISTS.md item #18. This rc completes the v6.23.0 swarm work begun in 6.22.2 (parallel `schedule_subagent` emission + parent-side lock) and 6.22.3 (fan-out telemetry + accept/reject markers). Dedup is unchanged. |
| 6.22.3 | 2026-06-08 | **feat(subagents): swarm fan-out telemetry and accept/reject markers.** Each `schedule_subagent` wave now emits one durable `swarm_fanout` event (`ouroboros/tools/control.py::_emit_swarm_fanout`) recording requested count, task group, role, requested/effective model lanes, depth, and inter-wave latency. The supervisor accept path tags scheduled subagents with `accepted=true` plus live `active_subagent_count`/`max_active_subagents`, and rejections are tagged `accepted=false` (`supervisor/events.py`). The Logs view summarizes `swarm_fanout` as a fan-out line (`web/modules/log_events.js`) without rendering a phantom child card. The emission tail is extracted into `_finalize_schedule_emission` and scheduled meta into `_subagent_scheduled_meta` to keep both functions within the size budget. The `ChatOutbound` gateway contract (plus its JSDoc mirror and the chat-history replay whitelist) gains optional `accepted`/`active_subagent_count`/`max_active_subagents` metadata so the markers survive reload; no `schedule_subagent` tool-schema change. |
| 6.22.2 | 2026-06-08 | **feat(subagents): parallel `schedule_subagent` emission with parent-side thread safety.** A burst of `schedule_subagent` calls in one tool-call round now runs in the existing tool ThreadPool instead of sequentially: `tool_capabilities.PARALLEL_SAFE_ENQUEUE_TOOLS` marks `schedule_subagent` as a fire-and-forget enqueue tool and `loop_tool_execution.tool_calls_can_run_parallel` parallelizes any round whose calls are all read-only-parallel or enqueue-safe. A process-local `_SCHEDULE_EMIT_LOCK` in `tools/control.py` serializes the parent-side `_last_scheduled_subagents` read-modify-write and the `pending_events` fallback so concurrent emission cannot lose records; the supervisor still drains its event queue serially, so cap/dedup/enqueue stay single-threaded. No tool-schema or gateway-contract change. |
| 6.22.1 | 2026-06-07 | **fix(runtime-subagents): classify hard provider failures and repair acting-subagent UX.** Provider calls now classify quota/auth/billing, hard bad-request, request-too-large, and context-overflow failures before retrying the same request, so Ouroboros surfaces recovery hints instead of burning rounds on identical calls. Typed task outcomes now separate errors from recoveries/warnings, so recovered tool failures and duplicate scheduling rejections do not paint successful tasks red. Acting external-workspace children are verified as shared-workspace edits instead of re-applying their patches, `plan_task` can resume an existing scout ledger, declared directory outputs become bounded manifest+zip artifacts, and workspace-backed `run_script` runs from workspace temp paths. The setup wizard is more compact, accounts for unsaved Anthropic keys before Claude runtime diagnostics, and Settings now shows mutative subagents as explicit On/Off while backend defaults still follow runtime mode. |
| 6.22.0 | 2026-06-07 | **feat(devtools-benchmarks): auditable benchmark ledgers and fail-closed smoke harnesses.** Benchmark devtools now share non-secret run manifests and denominator-preserving result ledgers across Terminal-Bench/Harbor, ProgramBench, SWE-bench, SWE-bench Pro, and OSWorld preflight paths. Terminal-Bench installed-agent smoke runs record observed official task IDs/rewards from Harbor outputs, validate explicit task filters, and keep provider secrets opt-in for local containers; ProgramBench, SWE/SWE-Pro, and OSWorld adapters write blocker sidecars instead of fake reruns when official harnesses, cleanroom preflights, prepared workspaces, or reviewed `computer_use` state are unavailable. External workspace patch artifacts now handle unborn git worktrees without synthetic commits, fail closed on invalid HEAD transitions, and block broader credential-shaped untracked files while allowing benign tokenizer/config artifacts. |
| 6.21.1 | 2026-06-07 | **fix(gemini-tools): drop empty-string enum that broke Gemini function-calling.** `schedule_subagent.write_surface` declared an enum with an empty-string member (`""` for "read-only"), which Google Gemini's function-calling validator rejects with HTTP 400 `INVALID_ARGUMENT` ("enum cannot be empty"). With `google/gemini-3.5-flash` as the main model, every turn exposing `schedule_subagent` failed on Gemini and fell back per-round to another provider, making multi-agent tasks slow/flaky and costly. The empty member is removed (read-only is expressed by omitting the optional param, already handled by `_select_subagent_constraint`), and a static guard test now fails if any tool-parameter enum contains an empty string (OpenAI/Anthropic accept empty enums, so this only surfaced against live Gemini). No other behavior change. |
| 6.21.0 | 2026-06-06 | **feat(acting-subagents-v2): UI toggle, genesis surface, best-of-N compare, and unified cleanup GC.** Adds the owner-facing master switch for mutative subagents as a tri-state Behavior control (Auto/On/Off, where Auto follows runtime mode); the agent still cannot self-enable it. New `write_surface=genesis` lets an acting child build a project from scratch — a fresh empty git repo auto-provisioned under a durable projects root (`OUROBOROS_SUBAGENT_PROJECTS_ROOT`), where the project directory itself is the deliverable (game/site/app/new Ouroboros) and is never GC-pruned or integrated into the live body. New read-only `compare_subagent_patches` shows several children's returned patches side by side for LLM-first synthesis. Subagent cards now show a `write=<surface>` badge. Cleanup retention is consolidated into ONE `OUROBOROS_GC_RETENTION_DAYS` knob (shared `age_cutoff`/`clamp_retention_days` helpers in `ouroboros/retention.py`) with a settings migration that folds the deprecated per-subsystem keys without orphaning a customized value. Real Windows portability fixes (not skips): backend path matching uses `is_absolute_path_text` so `/workspace/...` paths resolve on Windows, and the executor process-cleanup PID guard falls back to liveness when no command line is available (`process_command` is POSIX-only); the four previously-skipped `full-test (windows-latest)` tests are unskipped. |
| 6.20.1 | 2026-06-06 | **feat(acting-subagents): mutative subagents with parent-integrated patches.** Subagents can now be mutative ("acting"), not only read-only. `schedule_subagent(write_surface=...)` spawns a child that writes inside ONE isolated surface — `self_worktree` (a `git worktree` of this repo from the parent's base commit) or `external_workspace` — gated by the `OUROBOROS_ALLOW_MUTATIVE_SUBAGENTS` master toggle (default ON in advanced/pro, OFF in light). A machine-enforced authority envelope keeps acting children from committing, running review/runtime/skills lifecycle, enabling tools, or writing cognitive memory; `active_tool_profile` fails closed so a delegated subagent never inherits `self_modification`. Children return a `workspace.patch`; the parent reviews and integrates a chosen patch with the new `integrate_subagent_patch` tool (manifest-first, sha256-verified, 3-way apply, protected-path gated, `subagent_patch_verdict` artifact) and stays the sole committer of the live body — enabling best-of-N (accept one, synthesize several, or reject). Routing is top-only; `self_worktree` keeps protected-path write/shell discipline; orphan worktrees are pruned on startup. Active-child cap per root raised 3→6. Patch 6.20.1 fix-forwards `full-test (windows-latest)` portability (test-only: subagent-patch sha256 byte-parity on Windows, plus skips for pre-existing devtools/executor POSIX-path and process-group cases unrelated to acting subagents); no runtime behavior change. |
...
Older releases are preserved in Git tags and GitHub releases. The 6.0.0 through 6.17.0 rows, the 5.2.0 through 5.33.0-rc.6 rows, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
