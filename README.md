...
[![Version 6.25.0-rc.2](https://img.shields.io/badge/version-6.25.0--rc.2-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.25.0-rc.2 | 2026-06-09 | **fix(release): harden rc.1 code-intelligence release for CI/review.** Fix-forward after the rc.1 tag exposed Windows-only test gaps and review hardening: the ripgrep downloader now verifies GitHub SHA256 sidecars before bundling executable assets; `search_code` has focused fake-rg tests for allowed-file filtering and fallback parity; planning-swarm wait avoids sub-centisecond overshoot slices on Windows timers; `query_code` excludes protected artifacts before inventory construction and blocks acting-subagent `system_repo` roots; and docs clarify the bundled ripgrep runtime. The 6.25.0-rc.1 feature surface is unchanged. |
