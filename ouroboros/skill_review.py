...
def run_owner_attestation(
    ctx: Any,
    drive_root: pathlib.Path,
    skill: Any,
    content_hash: str,
) -> SkillReviewOutcome:
    """C1 (v6.39): owner-attested skill review — the owner explicitly skips the EXPENSIVE
    LLM review for their OWN skill. The DETERMINISTIC preflight floor STILL runs (an invalid
    manifest / sensitive-shaped / path-escaping payload can NEVER be attested); only the
    LLM phase is skipped. On a clean preflight, persist a durable CLEAN verdict bound to the
    content_hash (review_profile='owner_attested', reviewer_models=['owner_attestation'],
    one explicit PASS finding so the status serializes) and drop the owner-issued marker that
    ...
    # have caught: a parsed-but-structurally-invalid manifest (unknown type/runtime/permission,
    # a type=extension with no entry, a type=script with no scripts, scheduled_tasks without a
    # supervised task, ...) surfaces only as SkillManifest.validate() warnings. Owner-attestation
    # refuses on ANY such warning (the owner must fix it or run the full review).
    manifest = getattr(skill, "manifest", None)
    validate_warnings = (
    ...
    _append_skill_review_history(
        drive_root, skill.name, status=STATUS_CLEAN, content_hash=content_hash, findings=findings,
    )
...
def review_skill_owner_attest(ctx: Any, skill_name: str) -> SkillReviewOutcome:
    ...
    job tracking / payload shape (C1, v6.39)."""
    ...
    if str(getattr(skill, "source", "") or "") != "external" and not getattr(skill, "is_self_authored", False):
        return SkillReviewOutcome(
            skill_name=skill.name, status=STATUS_PENDING,
