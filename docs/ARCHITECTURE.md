# Ouroboros v6.24.0-rc.2 — Architecture & Reference
...
| OUROBOROS_POST_TASK_EVOLUTION | false | Owner-gated, default-OFF post-task self-evolution envelope (V4). When on, a qualifying task may promote ONE improvement into the existing gated evolution campaign. Owner-only: not settable via the generic `/api/settings` merge. v1 owner path is a manual `data/settings.json`/env edit (no dedicated endpoint/CLI yet); a dedicated Evolution settings UI control lands in a later phase. |
| OUROBOROS_POST_TASK_EVOLUTION_CADENCE | llm | Post-task evolution cadence: `off` \| `llm` (LLM decides per task) \| `every_n:<k>` |
