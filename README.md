...
[![Version 5.17.0-rc.3](https://img.shields.io/badge/version-5.17.0--rc.3-green.svg)](VERSION)
...
### Upgrade floor (very old installs)

From **5.17.x** onward, the app no longer performs automatic one-shot migrations for legacy layouts, including: unmarked skill payloads left under `data/skills/native/` (move them to `data/skills/external/` or reinstall), seeding scratchpad blocks from an old-only `scratchpad.md`, converting `dialogue_summary.md` into `dialogue_blocks.json` at context build time, copying env-only secrets into `settings.json` on first boot, standalone Git “token-in-URL → credential helper” passes beyond what `configure_remote()` already does, and silent rewrites of retired OpenRouter model id strings when only a remote key is configured. If you depended on those behaviors across very old trees, do a **clean reinstall** or **manual migration** (fix paths under `data/skills/`, recreate settings in the UI, update model ids) before expecting Repair, reviews, and provider defaults to line up.
...
|---------|------|-------------|
| 5.17.0-rc.3 | 2026-05-12 | **refactor(runtime+api): moderate RC cleanup — remove legacy migrations, DRY commit tail + HTTP errors, settings atomic JSON, web fetch alias.** Drops runtime skill topology migration (`skill_migrations`), scratchpad/dialogue/env/Git-token migrations, and retired-model id auto-rewrite for OpenRouter-only settings (README upgrade floor documents manual recovery). `configure_remote` remains the Git credential path; `_git_commit_success_tail` shares post-review `git commit` + forensic accounting between `repo_commit` and `repo_write_commit`; `json_error` consolidates simple API error envelopes across extensions/marketplace/file browser/local model; `save_settings` uses `atomic_write_json` with ASCII-preserving fallback; exports `jsonFetch` alias and routes lifecycle queue polling through `fetchJson`. |
| 5.17.0-rc.2 | 2026-05-12 | **fix(skills): expose executable review gate everywhere skill status is shown.** Adds a single `review_gate` / `executable_review` contract over skill review status, stale state, and the current review enforcement mode so agents, lifecycle messages, API payloads, and the Skills UI stop treating `status=advisory` as "passed" when blocking enforcement is active. `review_skill`, list/catalog/manifest responses, extension runtime state, marketplace installed payloads, and toggle responses now surface the same gate summary; review lifecycle headlines say `Review blocked` / `Review executable` instead of ambiguous `Review advisory`; Skills UI action logic consumes the gate with backward-compatible fallback; docs and SYSTEM.md now instruct agents never to call a skill ready unless `executable_review=true`. |
| 5.17.0-rc.1 | 2026-05-11 | **feat(review): share triad parsing and harden skill-review continuity.** Adds a shared triad-review module: repo review now reuses its JSON-array extraction while skill review uses its parser/quorum/actor-record/error-event helpers; makes `review_skill` and recent task context recovery available without silent truncation; exposes skill `review.json` as readable cognitive evidence while preserving write guards; computes skill review status live from findings + current enforcement mode; fixes `SKILL_NOT_FINALIZED` transcript injection so Anthropic requests end with a user turn; adds LLM-first `recent_tasks` recovery for continuation work; hardens skill review with rebuttals/history/convergence hints/preflight/degraded quorum events; clarifies advisory-status UX; and fixes MagicMock JSONL pollution from partial test contexts. |
| 5.16.0-rc.2 | 2026-05-11 | **chore(tests): prune ~16k LOC of obsolete or duplicative tests (61,246 → 45,076 test LOC; 24 files dropped, 2 trimmed to security-only).** Whole files removed: `test_pr_tools.py`, `test_ci_tool.py`, `test_skill_publish.py`, `test_local_model.py`, `test_build_repo_bundle.py`, `test_chat_js_contracts.py`, `test_git_pr_override_author.py`, `test_marketplace_adapter.py`, `test_marketplace_install.py`, `test_marketplace_skill_loader_migration.py`, `test_settings_runtime_regressions.py`, `test_settings_network_hint.py`, `test_settings_hot_reload.py`, `test_settings_ui_guards.py`, `test_settings_updates_ui.py`, `test_scope_review_block3.py`, `test_block1_review_pipeline.py`, `test_review_v4_33.py`, `test_review_calibration.py`, `test_review_synthesis.py`, `test_skills_marketplace_ui.py`, `test_advisory_workflow.py`, `test_reviewed_commit_workflow.py`, `test_loop_checkpoint.py`. Trimmed to security-only: `test_chat_logs_ui.py` (fixture-backed `test_static_ui_contracts` retained, per `docs/ARCHITECTURE.md` reference and the `tests/fixtures/chat_logs_ui_static_checks.json` invariant); `test_marketplace_clawhub.py` (host allowlist, redirect handler, response-size cap, slug traversal). Preserved intact: P1/P3/Panic/security/skill-trust core — `test_contracts.py`, `test_runtime_mode_core.py`, `test_runtime_mode_elevation.py`, `test_safety_policy.py`, `test_skill_exec.py`, `test_skill_loader.py`, `test_extension_loader.py`, `test_review_fidelity.py`, `test_review_observability.py`, `test_smoke.py` AST gates, `test_marketplace_fetcher.py`, `test_platform_guard.py`, `test_packaging_sync.py`, panic/shutdown/zombie suites. Stale references cleaned in `web/modules/settings.js` and `tests/test_launcher_sync.py` (both pure comment-only edits — no runtime behavior change). Full pytest still green (2,586 tests collected, 2,585 passed, 1 skipped); marker lanes (`browser`, `ui_browser`, `ui_browser_docker`, `portable_detail`, `integration`) all non-empty. No runtime behavior changes under `ouroboros/` or `supervisor/`. VERSION 5.16.0-rc.1 → 5.16.0-rc.2 (P9: PATCH-class within the current RC train); `VERSION` / `pyproject.toml` (`5.16.0rc2`) / README badge + Version History / `docs/ARCHITECTURE.md` header all synced. Oldest visible minor row (`5.15.0-rc.10`) rolled off to respect the P9 changelog cap. |
| 5.16.0-rc.1 | 2026-05-11 | **feat(skills+light): two skill-lifecycle UX fixes — submit marketplace-managed skills to OuroborosHub + light-mode bucket+skill_name short-form authoring.** (A) Removed the dual whitelist guard that blocked ClawHub/OuroborosHub-managed skills from `submit_skill_to_hub`; the UI button and backend validation now both accept marketplace sources, so users can PR fixes/adaptations of marketplace-installed skills back to OuroborosHub. The PR body auto-prefixes a `## Provenance` block reading the local sidecar (`.ouroboroshub.json` `slug` / `.clawhub.json` `clawhub_slug`); the prefix is force-applied to both the LLM-generated body and the fallback, and is dropped silently when no sidecar exists (soft fallback — skill_loader already reclassifies sidecar-less skills as `external`, and submit then routes through the existing external flow). Existing safety gates (PASS review, non-stale hash, version bump, GITHUB_TOKEN, secret scan, payload size cap, control-plane sidecar filter) are unchanged. (B) Under `runtime_mode=light`, the three skill-payload edit tools — `data_write`, `str_replace_editor`, `claude_code_edit` — now accept optional `bucket` + `skill_name` args (enum `external` / `clawhub` / `ouroboroshub`; `native` excluded). When both are supplied, a short relative `path`/`cwd` (e.g. `plugin.py`, `lib/utils.py`, `.`) resolves under `data/skills/<bucket>/<skill_name>/` exactly as a `task_constraint.mode='skill_repair'` task would — the new helper `synthesize_payload_constraint` in `ouroboros/contracts/skill_payload_policy.py` synthesizes the same `skill_repair`-flavoured constraint (no new mode is introduced). Both the registry-level light gate and each of the three handlers consult the synthesised constraint; partial args (only `bucket` or only `skill_name`) return a clear error rather than silently writing into the drive root. Sidecar control-plane protection (`allow_control_plane=False`) is preserved end-to-end. The `LIGHT_MODE_BLOCKED` message now enumerates all three valid escape hatches (`skill_repair`, explicit `data/skills/<bucket>/<skill>/...`, `bucket+skill_name` args) so agents stop falling back to `data_write` heredocs. `prompts/SYSTEM.md` and `docs/CREATING_SKILLS.md` document both the short-form and the large-file strategy (`data_write(mode='append')` in chunks, `claude_code_edit` for internal subdivision; bash heredoc does NOT bypass the LLM output token budget). VERSION 5.15.0-rc.13 → 5.16.0-rc.1 (P9: MINOR — new capabilities); README badge + Version History, `pyproject.toml [project].version` (5.16.0rc1), `docs/ARCHITECTURE.md` header + `skill_publish.py` / `skill_payload_policy.py` descriptions, and `docs/DEVELOPMENT.md` light-skill-authoring paragraph all synced. Full pytest green locally. |
Older releases are preserved in Git tags and GitHub releases. The 5.2.0 through 5.14.0 stable rows (and every 5.x rc tag prior to 5.16.0-rc.1, including v5.15.0-rc.1 through v5.15.0-rc.13) and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
