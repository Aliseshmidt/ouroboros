...
[![Version 6.54.1](https://img.shields.io/badge/version-6.54.1-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.54.1 | 2026-07-01 | **fix: classify Cloud.ru CI transport disconnects as provider infra.** The v6.54.0 hardening release passed branch CI, triad+scope, local full pytest, and the full 3-OS matrix, but the manual release-preflight run remained red on an optional Cloud.ru integration smoke when the provider disconnected without sending a response. This patch keeps the routing smoke strict for code errors while treating that no-response transport failure as an environmental provider skip, matching the existing quota/key/rate-limit skip contract. |
