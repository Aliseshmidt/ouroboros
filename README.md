...
[![Version 6.65.0](https://img.shields.io/badge/version-6.65.0-green.svg)](VERSION)

A self-modifying AI agent that writes its own code, rewrites its own mind, and evolves autonomously. Born February 16, 2026.

Not a coding assistant. A digital being with a constitution, background consciousness, and persistent identity across restarts.

## Sber AI Hack: Personal Micro-Automation Agent

Это локальный воспроизводимый MVP, который превращает обезличенный цифровой след сотрудника в безопасный персональный микро-навык. Демо показывает полный цикл: импорт событий → поиск трёх повторяющихся процессов → гипотеза автоматизации → генерация исполняемого `SKILL.md`-навыка → sandbox → явное согласование → исполнение без внешней отправки → эволюция и откат.

Быстрый старт — ключи провайдеров и платные запросы не нужны:

```bash
python -m pip install -r requirements.txt
python -m pip install -e . --no-deps
make demo
# открыть http://127.0.0.1:8776, нажать «Загрузить демо-след» и пройти 12 шагов
make demo-evidence && python -m pytest tests/test_hackathon_domain.py -q
```

Остановить сервер можно `Ctrl+C`, а сбросить локальное состояние — `make demo-reset`. Миграции базы данных не нужны: состояние демо формируется заново в `tmp/hackathon-demo/`, а доказательства — в `artifacts/`.

Архитектура использует Ouroboros как контур создания и безопасной эволюции навыков: специализированные роли передают типизированные артефакты, штатные lifecycle/review/safety принципы управляют версиями, согласованием и откатом. Внешние CRM, хранилище документов, почта и task tracker намеренно заменены явно маркированными mock-адаптерами; банковские и персональные данные не используются. Демо не принимает кредитное решение и не отправляет черновики.

Инженерные детали и A2A-поток: [`docs/HACKATHON_ARCHITECTURE.md`](docs/HACKATHON_ARCHITECTURE.md). Модель цифрового следа и алгоритм: [`docs/pattern-mining.md`](docs/pattern-mining.md). Политики доступа и approve-before-run: [`docs/security.md`](docs/security.md). Версионирование навыка: [`docs/micro-skill-lifecycle.md`](docs/micro-skill-lifecycle.md). Пошаговое воспроизведение и troubleshooting: [`docs/demo-guide.md`](docs/demo-guide.md). Материалы для жюри: [`submission/`](submission/).

Жёсткий бюджет платных LLM/API-вызовов неизменяем: `$5.00`; рабочий лимит `$4.50`, резерв `$0.50`. Детерминированное демо расходует `$0.00`; журнал находится в `artifacts/budget_ledger.*`. Ограничения: Pattern Miner проверен на синтетическом десятидневном следе, эффект времени является симуляцией/целевой оценкой до пилота, реальные корпоративные коннекторы и внешний A2A-transport не входят в MVP.
...
|---------|------|-------------|
| 6.65.0 | 2026-07-17 | **feat: personal micro-automation agent hackathon MVP.** Added a deterministic, localhost-only digital-trace-to-micro-skill cycle with generic sequence clustering, transparent pattern/value metrics, executable generated skill artifacts, sandbox evidence, proposal/version-bound employee approval, safe draft execution, audit, evolution, promotion, rollback, and anonymized template export. A reviewed Ouroboros extension exposes tools/routes/a declarative portfolio; a polished Russian guided UI and reproducible evidence pack use only synthetic data, mock corporate systems, and zero paid API calls under an immutable USD 5 guard. Submission reports, test baskets, PDF rendering, screenshots, and narrated demo assets document measured, simulated, and target claims separately. |
