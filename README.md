...
|---------|------|-------------|
| 5.17.0-rc.3 | 2026-05-12 | **refactor(runtime): retire unsupported legacy migrations and centralize JSON state IO.** Removes old native-skill topology, flat memory, launcher env-settings, and token-in-URL credential migration paths now covered by the v5.17 upgrade floor; documents manual upgrade guidance for very old layouts; replaces duplicated JSON reads/writes with shared `read_json_dict` / `atomic_write_json` where safe; and hardens corrupt startup-state handling so budget, crash-report, and restart-verify failures stay visible. |
