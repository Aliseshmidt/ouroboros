...
[![Version 6.0.1](https://img.shields.io/badge/version-6.0.1-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.0.1 | 2026-05-08 | **fix(host-service): avoid blind port kills on normal shutdown.** Host Service teardown now relies on uvicorn's graceful in-process shutdown during ordinary lifespan exits; emergency and panic paths still own forceful process/port cleanup, avoiding accidental termination of unrelated local processes while preserving stop safety. |
| 6.0.0 | 2026-05-08 | **feat(skills): move A2A and Telegram into reviewed transport skills.** A new loopback-only Host Service API, companion-process supervisor, event bus, opaque skill tokens, permission grants, and chat-id policy let reviewed skills inject transport messages without patching the core. Base-runtime A2A modules/deps and Telegram polling code are removed, Settings gains a unified Secrets surface, and this core release is paired with an external OuroborosHub catalog update that publishes the official `a2a` and `telegram-bridge` skills. Host Service, companion, grant, event-bus, and transport-extraction tests cover the new flow. |
...
Older releases are preserved in Git tags and GitHub releases. The 5.2.0, 5.3.0, 5.3.x release-candidate, 5.4.0, 5.5.0, 5.7.5, 5.7.6, 5.8.0-rc.2, 5.8.0-rc.6, 5.8.1, 5.8.2, 5.8.3-rc.1, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
