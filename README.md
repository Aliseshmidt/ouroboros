...
[![Version 5.24.0-rc.1](https://img.shields.io/badge/version-5.24.0--rc.1-green.svg)](VERSION)
...
| 5.23.0-rc.2 | 2026-05-16 | **rc(ci): tolerate Cloud.ru reasoning-only smoke responses.** Keeps provider integration green when Cloud.ru's hosted GLM model returns usage and reasoning but no final `content` for the minimal routing smoke prompt, treating that as an environmental/model-output quirk rather than a routing regression. |
| 5.23.0-rc.1 | 2026-05-16 | **rc(review): relax checklist coverage and preflight release metadata before advisory.** Allows review outputs to preserve multiple distinct FAIL entries under the same checklist item, keeps duplicate/order advisory contract drift as warnings with raw evidence, fixes reviewer obligation-suffix normalization, auto-syncs release carriers before advisory spend, blocks missing VERSION/P9 release drift before provider calls, and treats successful grep regex auto-correction as a non-error tool result. |
| 5.21.0 | 2026-05-14 | **feat(safety+widgets): harden unified edit targeting and repair Widgets host flows.** Makes `bucket`/`skill_name` short-form skill edits safe around explicit repo/data paths, adds a light-mode repo mutation tripwire for shell/Claude edit paths without auto-rollback, surfaces runtime mode in task context, restores widget span metadata, renders `subscription.render[]` progress/media children for Video/Anime Studio widgets, makes selected skill-review findings severity-driven, and pins budget hot-reload behavior with regression coverage. |
...
Older releases are preserved in Git tags and GitHub releases. The 5.2.0 through 5.19.0 rows and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
