# Ouroboros v6.37.1 — Architecture & Reference
...
| `task_contract` — canonical host-draft task objective/output/constraint/resource/deadline/workspace/lineage/delegation-budget contract helpers (`build_task_contract`, `attach_task_contract`, `normalize_allowed_resources`, `normalize_resource_policy`, `normalize_delegation_budget`). v6.37.0 adds the additive `delegation_budget` block (`may_delegate`/`may_mutate`/`may_fan_out`/`depth_remaining`/`max_children`/`intent_note`) so a parent's delegate/mutate/fan-out intent propagates structurally to children. | `ouroboros/contracts/task_contract.py` | `tests/test_contracts.py::test_public_api_is_stable` pins the public helper names (subset); task/outcome tests pin resource and resource-policy normalization plus contract propagation; `tests/test_delegation_budget.py` pins the budget block |
