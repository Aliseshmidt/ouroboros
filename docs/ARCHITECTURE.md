# Ouroboros v6.64.0 — Architecture & Reference
...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── pricing.py           ← Provider-specific linear price lookup and compatibility usage projections; it is not the monetary ledger
      ...
      ├── project_dialogue.py  ← Read-only canonical Project dialogue lens plus append-only presentation annotations (`logs/chat_annotations.jsonl`); projects reference original chat rows instead of copying/mirroring them, and the sidecar never owns routing or Project state
      ...
      ├── review_substrate.py  ← Reviewer-slot coordinator used by task acceptance and planning helpers; duplicate model ids remain independent slots. Task acceptance enforces adaptive quorum, one substantive call and no more than two physical attempts per actor, metric-grounded criterion evidence, provenance, and a public-info-only anti-cheat boundary. Commit/triad/scope P3 orchestration remains a separate one-pass contract.
...
│   │   ├── project_task_bindings.json ← Immutable task→project bindings plus canonical source-row refs; one root can belong to at most one Project and tombstoning never removes the binding
...
The configured task-acceptance panel uses independent reviewer slots and `adaptive_quorum` (1→1, 2→2, 3+→2). Each actor gets one substantive call and at most two physical sends total (same-route transport retry or extraction/format repair); a third send is blocked. Any valid contributing FAIL fails the aggregate, DEGRADED abstains, and no quorum becomes `review_degraded`, never a false PASS. Clean means quorum PASS, `solved`, and supported evidence for every contributing criterion. Actionable gaps are exact-deduplicated into the existing obligation/improvement path. Required+Advisory may finalize with an honest non-clean verdict; Required+Blocking keeps iterating until clean or a real deadline/budget/lifecycle rail, subject to any explicit `max_improvement_passes`. The first review reserves at least 200 seconds; later passes use `max(configured floor, 1.5×timing EWMA)` with `alpha=0.5` reconstructed from existing events. Lifecycle remains `completed` when an artifact exists; objective/review status and stop reason carry the warning. Only the root performs global post-task synthesis, once, and `root_phase_checkpoint` provides the minimal restart seam without a parallel admission journal: startup replays only `pending_once`; an indeterminate persisted `running` phase is terminally disclosed as degraded instead of risking a second paid sequence.

The existing agent-callable `task_acceptance_review` and legacy `agent_disposition`/`obligation_dispositions` inputs remain one-minor compatibility/advisory surfaces; they do not override the root authority. Task acceptance and the P3 commit gate intentionally remain different systems.
...
Prompt caching and reasoning continuity are provider-gated in `llm.py`. Stable governance/policy is placed before dynamic evidence. Anthropic-compatible routes retain supported ephemeral message/tool cache markers; direct OpenAI may additionally receive a stable-prefix `prompt_cache_key`, and OpenRouter a conversation-stable `session_id`. If the exact provider explicitly rejects that named cache parameter, the same request is retried once without it; other errors do not trigger this fallback. Direct/local payloads strip OpenRouter-only reasoning metadata, while OpenRouter retains its existing portable-family continuity and same-model provider-resilience rules. This narrow cache-affinity support does not introduce body rerouting, provider hopping, or an internal retry platform.
...
Such a physical rejection is fail-closed under the default `blocking_1m` floor:
there is no authoritative verdict. Only an explicit owner-selected `advisory`
scope floor keeps the visible non-blocking `budget_exceeded` signal. So
...
input cap. A known sub-1M reviewer remains advisory-only: under the default
`blocking_1m` floor its result is preserved but cannot satisfy the gate; only an
explicit owner-selected `advisory` floor keeps a non-blocking `budget_exceeded`
skip. The same authority rule applies if the estimate-based gate passes but the
...
configure a larger-window reviewer). The non-blocking `budget_exceeded` skip
and provider-oversize advisory are reserved for the explicit owner-selected
advisory floor; they never satisfy the default P3 gate. The P3-aligned remedy
...
`OUROBOROS_SCOPE_REVIEW_DEGRADED=true` retains the explicit low-context advisory
builder for diagnostics, but the P3 commit orchestrator never launches it as a
second substantive reviewer call. The one-pass gate returns the normal actor's
authoritative or fail-closed status. A separately requested degraded diagnostic
uses a smaller effective input limit, reports uncovered files, downgrades any
critical findings to advisory, and appends `scope_review_degraded`; it never
replaces or participates in the blocking 1M scope authority.

### Planning, deep review, reflection, memory

`plan_review.py` runs the existing multi-model Atlas-backed panel before large implementation plans. One or two stable read-only planning scouts use the normal subagent pool, persist their full raw handoff artifact, and pass compact findings plus that ref to the reviewer prompt; each configured planning actor still receives one substantive call. Governance documents always come from the system repo, while planned file snapshots and Atlas inventory come from `active_repo_dir_for(ctx)`. A missing/mixed subject root or any planned path escaping that root fails loudly instead of silently reviewing Ouroboros in place of an external workspace. The prompt carries one canonical intent/evidence horizon (task contract, aliases, forensic refs, omissions), not replicated copies or a second planning engine; `scripts/run_plan_review.py` remains a thin operator wrapper over this production panel.
...
Startup runs one resumable importer with a source-hash watermark. It writes immutable copies of legacy `events.jsonl` and `state.json`, records their hashes plus a before hash of `settings.json`, imports only real usage rows as attempts, keeps ambiguous calls metadata-only, represents any state-vs-event monetary remainder as a disclosed legacy delta, never rewrites source logs, and does not fabricate attempts. Secret settings contents are neither archived nor modified. Any one-host forensic repair of an already completed watermark is an operator migration, not a second runtime reconciliation API.
...
| OUROBOROS_CONTEXT_MODE | max | Owner-selected context horizon: `max` or `low`. Ordinary Main calls build deterministic route-calibrated projections from one immutable core; an unknown route tries Max (never a silent 200K fallback), and a confirmed overflow may retry the same model once with task-local Low without mutating this global setting. P3 commit review is excluded. Plain `/api/settings` POST drops this key; owner endpoints/CLI control it. |
| OUROBOROS_SCOPE_REVIEW_DEGRADED | false | Retained explicit diagnostic builder for a separately requested low-context advisory report. The one-pass P3 commit orchestrator never auto-launches it as a second substantive scope call, and it can never satisfy the blocking 1M floor. |
