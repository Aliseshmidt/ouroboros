...
skills/computer_use/        ← Bundled extension skill payload for supervised local desktop observation/input (screenshot, window_list, click/type/key/move/scroll, best-effort AX summary). It is reviewed/enabled through the normal skill lifecycle, reports missing platform backends instead of guessing, and is not required by the OSWorld step-loop adapter.
