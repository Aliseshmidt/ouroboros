...
[![Version 6.24.0-rc.2](https://img.shields.io/badge/version-6.24.0--rc.2-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.24.0-rc.2 | 2026-06-08 | **feat(self-evolution): SWE-bench Pro evolutionary driver — cross-task self-evolution release (C3 / Phase 6).** (rc.2 fixes a Windows CI test-portability bug: `test_evolve_pro.py`'s bash guard now verifies bash actually runs, since Windows `bash.exe` is a WSL launcher stub that fails when no distro is installed and `capture_patch.sh` is a POSIX/Docker devtool.) Capstone of the cross-task self-evolution line (6.23.1 post-task envelope, 6.23.2 backlog overhaul, 6.23.3 per-project facts, 6.23.4 cleanup, 6.23.5 settings UI). Adds `devtools/benchmarks/swe_bench_pro/evolve_pro.py`: an isolated evolutionary driver that solves SWE-bench Pro instances in sequence on a throwaway Ouroboros clone + isolated data root, running ONE post-task self-evolution cycle between instances (the C1 envelope) so learned memory and reviewed self-modifications carry forward while the live body is never touched. Per instance it checks out `base_commit`, runs standalone `ouroboros run --workspace <repo> --memory-mode forked` (external workspaces forbid `shared`; forked still carries the isolated canonical memory), captures a `grade_pro`-compatible `model_patch`, and resets the per-task budget (guarded; isolated root only) so a fresh instance is never falsely flagged `budget: emergency`. `--demo N` exercises the whole loop with no dataset/Docker; `METHODOLOGY.md` documents the protocol and the official Pro scorer remains source of truth. |
