...
[![Version 5.15.0-rc.4](https://img.shields.io/badge/version-5.15.0--rc.4-green.svg)](VERSION)
...
|---------|------|-------------|
| 5.15.0-rc.4 | 2026-05-10 | **refactor(web): consolidate skill-repair prompt + lifecycle event helper + alert→toast SSOT.** Web/JS dedup pass. `web/modules/utils.js` gains `emitSkillLifecycle()` and `renderSkillRepairPrompt(intro, diagnosticsJson)` — single sources of truth for the `ouro:skill-lifecycle` CustomEvent (was duplicated in `marketplace.js` and `ouroboroshub.js`) and for the My Skills / ClawHub Marketplace heal-prompt narrative (the "Tool choice" / "untrusted data" body was copy-pasted in two slightly-different forms). Replaced 9 native `alert()` calls in `settings.js`, `updates.js`, `settings_local_model.js`, and `chat.js` with `showToast()` from the v5.13.0 notification SSOT — completes the design-system migration started in v5.13.0. Skipped (high JS risk for modest LOC): `renderMarketplaceCardLayout`, `log_events.summarizeLogEvent` data-table rewrite, `widgets.js` legacy iframe branches. Net **−21 LOC** across 9 files; UI static tests still green. Visual UI parity preserved (same lifecycle event payload, same heal-prompt text, toasts replacing modal alerts). |
| 5.15.0-rc.3 | 2026-05-10 | **refactor(legacy): retire pre-v5.5 / pre-v5.0 compatibility shims.** Targets the deferred legacy/compat surface in the v5.14.0 plan that was never executed. Removes `migrate_generation_skill_names` + `_RENAME_SPECS` + `_rewrite_text_files` + `_looks_like_known_legacy_skill` from `skill_migrations.py` (the image_gen → nanobanana / audio_gen → music_gen migration window for users upgrading from pre-OuroborosHub builds is closed); the `migrate_unseeded_native_skills_to_external` topology repair stays. Retires the `OUROBOROS_INITIAL_REASONING_EFFORT` legacy alias from `config.resolve_effort` + the dead read in `web/modules/settings.js` + tests. Deletes the unused `get_clawhub_enabled` helper from `config.py` (no production caller) and the `OUROBOROS_CLAWHUB_ENABLED` environment variable plus the dead `enable_marketplace` fixture in `tests/test_marketplace_install.py` + the misleading patches in `tests/test_marketplace_provenance_contract.py`. Net **−163 LOC** across 11 files; 3373 tests still green (3 obsolete legacy tests removed). |
| 5.15.0-rc.2 | 2026-05-10 | **refactor(http+ssot): centralise HTTP-API plumbing and finish UTC + governance dedup.** New `ouroboros/http_api.py` provides one set of `request_drive_root` / `request_repo_dir` / `coerce_bool` / `coerce_int` / `json_error` helpers; `extensions_api`, `marketplace_api`, `file_browser_api` and `server.py` drop their private copies. UTC ISO timestamp generation routes through `utc_now_iso()` everywhere it was still ad-hoc (`datetime.now(timezone.utc).isoformat()` removed in `skill_loader`, `skill_review`, `server_runtime`, `review_state`, `launcher_bootstrap`, `marketplace/{adapter,ouroboroshub,provenance,isolated_deps}`, `server.py`). Governance-doc loading dedup: `_load_bible` in `tools/plan_review` and `_load_governance_artifact` in `skill_review` now thin-wrap `tools/review_helpers.load_governance_doc` so missing-doc OMISSION markers are emitted in one canonical format. Net **near zero LOC** as the new helper module's surface area roughly matches the per-call savings; the architectural win is one place to evolve HTTP-API contracts. 3376 tests still green. |
| 5.15.0-rc.1 | 2026-05-10 | **refactor(tests): aggressive consolidation pass — fixtures + parametrize.** First catch-up RC of the v5.15.0 reduction line. `tests/_shared.py` centralises the extension-loader runtime cleanup (three duplicated copies in `test_skill_exec` / `test_extensions_api` / `test_extension_loader` now collapse onto one helper) and the `claude_agent_sdk` mock installer (three duplicated copies in `test_commit_gate` / `test_advisory_observability` / `test_claude_code_gateway` now share one). `tests/conftest.py` drops four unused fixtures (`make_git_repo`, `tool_context`, `make_chat_mock`, `make_extension_skill`) that no test ever requested. Per-file parametrization replaces 13 UI-tab rejection tests + 3 route-rejection tests in `test_extension_loader`, the 11-case preflight cluster + 4-case JSON parser cluster in `test_phase7_pipeline`, the 3-case truncation suite in `test_review_fidelity`, the resolve-claude-code-model trio + budget-gate trio (now sharing one `budget_gate_env` fixture) in `test_advisory_observability`, the registered-tools triplet in `test_commit_gate`, and the 37 paired `_make_temp_git_repo + _make_ctx` setups in `test_pr_tools` (single `_setup` helper). Net **−312 LOC** with 3376 tests still green. |
...
Older releases are preserved in Git tags and GitHub releases. The 5.2.0, 5.3.0, 5.3.x release-candidate, 5.4.0, 5.5.0, 5.6.0, 5.7.0, 5.7.5, 5.7.6, 5.8.0-rc.2, 5.8.0-rc.6, 5.8.1, 5.8.2, 5.8.3-rc.1 through 5.8.3-rc.5, 5.9.0, 5.9.1, 5.9.2, 5.10.0 through 5.10.4, 5.11.0, 5.12.0, 5.13.0 (still listed above), 5.14.0-rc.1 through 5.14.0-rc.3, and former `4.0.0` rows are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
