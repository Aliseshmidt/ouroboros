...
[![Version 6.24.0-rc.3](https://img.shields.io/badge/version-6.24.0--rc.3-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.24.0-rc.3 | 2026-06-08 | **fix(self-evolution): v2 hardening of the cross-task self-evolution release (C3 / Phases A–G), after full adversarial + real triad+scope review.** Capstone hardening of the 6.24.0 self-evolution line (6.23.1 post-task envelope, 6.23.2 backlog, 6.23.3 per-project facts, 6.23.4 cleanup, 6.23.5/rc.1/rc.2 settings + driver). The SWE-bench Pro evolutionary driver is rebuilt **server-driven**: instead of `ouroboros run` (which silently attached to the live server when one was up, so the agent never actually self-evolved), `evolve_pro.py` and `evolve_smoke.py` now spawn a REAL isolated `server.py` on an OS-assigned free port against a throwaway clone + isolated data root via the new shared `devtools/benchmarks/common/server_runner.py`, faithfully driving the production server loop (with `platform_layer` process-group teardown and an idle/no-promotion early exit); the clone's `origin` (== live repo) is dropped so an isolated self-mod commit can never `_auto_push` back to the live body. (Owner-deferred for rc.3: real cross-task self-evolution between benchmark instances does not yet fire — instances are project-scoped external workspaces, which the Phase-3 leak guard intentionally excludes from post-task promotion; the driver delivers isolation + solve + grade-compatible capture + budget reset, and the evolve-between-instances depth is a tracked follow-up, see `swe_bench_pro/METHODOLOGY.md` §3.) Subagent **browser/VLM** is relaxed for readonly/acting subagents — scoped `file://` under the workspace plus loopback HTTP(S) EXCEPT Ouroboros control-plane ports (now including `LOCAL_MODEL_PORT` and the actual bound `state/server_port`, not just 8765–8767), plus `vlm_query` — while keeping SSRF/private-IP/DNS-rebind blocks; a `page.evaluate` `<<` SyntaxError now returns an actionable hint. The post-task evolution **settings group** is completed: cadence as Off/LLM/Every-N buttons with strict `every_n:<k>` validation on both the backend normalizer and the Save path (malformed N aborts the save instead of silently coercing to every-task), budget reserve, persistent objective (additive/non-escalating owner steer), and BG_* restart-required keys. `OUROBOROS_POST_TASK_EVOLUTION` is now owner-toggleable in the UI yet stays self-enable-proof via shell + browser-JS detectors, a `POST /api/settings` route guard, and a SAFETY.md bullet. D5 decouples the recorded `memory_mode` from drive materialization; `project_id` joins the frozen `ToolContextProtocol`. Default behavior unchanged; identity stays candidate-only; the official Pro scorer remains source of truth. |
