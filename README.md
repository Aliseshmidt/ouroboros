...
[![Version 6.14.0-rc.1](https://img.shields.io/badge/version-6.14.0--rc.1-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.14.0-rc.1 | 2026-06-03 | **feat(provider): add GigaChat as a first-class provider.** Adds Sber GigaChat (`gigachat::` model values) routed through the native `gigachat` library instead of the OpenAI SDK, since GigaChat is not OpenAI-compatible: OpenAI tools map to GigaChat functions with a single `function_call` per turn, the system message stays first while later system-reminders are demoted to user, and function results are JSON-wrapped; `reasoning_effort` is omitted on the GigaChat path so replies are not consumed by hidden reasoning. A GigaChat-only install is self-sufficient: model slots and the review and scope reviewer slots all resolve to `gigachat::` models, with the audited degraded scope-review path covering the sub-1M context floor, and this single-isolated-provider expectation is documented as an invariant in DEVELOPMENT.md and the review checklist. Real Sber tariffs back cost tracking and the `gigachat` dependency is pinned below 0.3. |
...
| 6.12.0 | 2026-06-03 | **release: QA hardening.** Marketplace auto-renames a ClawHub skill whose name collides with another bucket (e.g. native `weather` → `weather-clawhub`) and maps registry 404→404/429→429; instruction skills gain an LLM-first "Make runnable" repair affordance. Scope review reserves output headroom inside the 1M reviewer window (the 920K prompt SSOT is unchanged) so an oversize prompt routes to the existing non-blocking skip instead of a hard provider 400. `/evolve stop` now breaks the hard-timeout auto-retry chain and cancels the running evolution worker; abnormal task termination reconstructs cost/rounds from durable `llm_usage` instead of recording zeros. The pytest preflight guarantees process-tree teardown (no orphaned CPU hogs) with a configurable `OUROBOROS_PREFLIGHT_TIMEOUT_SEC`, and worker log lines now stream to the dashboard with reload backfill. |
| 6.11.0-rc.2 | 2026-06-02 | **rc(evolution): finish release-gate fixes.** Fixes the prerelease tag workflow regressions from rc.1 by making Claude advisory child signal names stable on Windows and aligning UI smoke's child server `OUROBOROS_SETTINGS_PATH` with its isolated data directory, so tag-triggered full matrix and smoke checks exercise the intended configured runtime instead of onboarding. |
| 6.11.0-rc.1 | 2026-06-02 | **rc(evolution): stabilize autonomous self-modification.** Adds hermetic reviewed-change pytest preflight in disposable worktrees, blocks tests from mutating live repo/data, preserves active evolution work with transaction-linked rescue refs instead of autonomous reset, treats reviewed local commits as the durability boundary with optional `origin` push, and isolates Claude SDK advisory crashes so child aborts return structured review errors instead of killing workers. |
